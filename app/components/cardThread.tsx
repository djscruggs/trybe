...
export default function CardThread (props: CardThreadProps): JSX.Element {
  ...
  const { isReplyTo, hasLiked } = props
  const [thread, setThread] = useState(props.thread)
  const [showLightbox, setShowLightbox] = useState(false)
  const [editing, setEditing] = useState(false)
  const location = useLocation()
  const isOwnRoute = isReplyTo ?? location.pathname === `/threads/${thread.id}`
  ...
  return (
    ...
    {editing
      ...
      <FormThread thread={thread} onCancel={() => { setEditing(false) }} afterSave={afterSave} />
        {thread.challenge &&
          <div className='mt-2'>
            <CardChallenge challenge={thread.challenge} isShare={true}/>
          </div>
        }
      </>
          ...
          <Card className={'md:col-span-2 p-2 border-1 drop-shadow-lg  border border-gray rounded-md'}>
            <div className="flex items-start">
              ...
              <div className="flex flex-col w-full h-full">
                ...
                {thread.post &&
                  <div className='mt-2'>
                    <CardPost post={thread.post} isShare={true}/>
                  </div>
                }
            ...
            {thread.replyTo && thread.isShare &&
              <div className='mt-6 ml-10'>
                <CardThread thread={thread.replyTo} isReplyTo={true} />
              </div>
            }
          </Card>
        </div>
      ...
      <div className='mt-2 w-full border-0  drop-shadow-none mr-2'>
        <FormThread replyToId={thread.id} afterSave={afterSave} onCancel={() => { setAddReply(false) }} prompt='Add your response' />
      </div>
      }
      ...
      <>
        ...
        <div className="grid grid-cols-3 text-center py-2 cursor-pointer">
          ...
          <Link to={`/threads/${thread.id}`}>
            ...
            <span className="text-xs">{thread._count?.replies} replies</span>
            </Link>
          </div>
          <div className="flex justify-center items-center cursor-pointer">

          <div className='mr-2'><Liker isLiked={Boolean(hasLiked)} itemId={Number(thread.id)} itemType='thread' count={Number(thread._count?.likes)}/></div>
          </div>
          <div className="flex justify-center items-center cursor-pointer">
